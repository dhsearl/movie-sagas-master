...
router.delete('/:id/:genre', (req,res)=>{
    if(verbose)console.log("in delete", req.params.id, req.params.genre);
    const queryText = `
    ...
    if(verbose)console.log("query args are", queryArguments)
    pool.query(queryText, queryArguments)
    .then(()=>{
        if(verbose)console.log('DELETE Success');
        res.sendStatus(200);
    })
    .catch((error)=>{
        if(verbose)console.log('Error in Delete Query', error);
        res.sendStatus(500);
    })
})
...
router.get('/', (req, res) => {
    ...
    pool.query(queryText)
        .then((result) => {
            if(verbose)console.log('/genres GET success', result.rows);
            res.send(result.rows);
        })
        .catch((error) => {
            if(verbose)console.log('Error in movie.router genre GET', error);
            res.sendStatus(500);
        })
})

router.get('/by/:genre', (req, res) => {
    if(verbose)console.log('movie.router by Genre Name GET hit', req.params.genre);
    const queryText = 
...
router.get('/of/:title', (req, res) => {
    if(verbose)console.log('GET GENRES OF MOVIE hit', req.params.title);
    const queryText = 
    ...
    pool.query(queryText, queryArguments)
        .then((result) => {
            if(verbose)console.log('/genres/of GET success', result.rows);
            res.send(result.rows);
        })
        .catch((error) => {
            if(verbose)console.log('Error in movie.router GET', error);
...
//      This is connected to the Set Genres Present Reducer
    if(verbose)console.log('movie.router genres present GET hit');
    const queryText = 
    ...
    pool.query(queryText)
        .then((result) => {
            if(verbose)console.log('/genres/present GET success', result.rows);
            res.send(result.rows);
        })
        .catch((error) => {
            if(verbose)console.log('Error in movie.router genre GET', error);
...
router.put('/add', (req,res)=>{
    ...
    //   
    if(verbose)console.log('IN GENRE/ADD route');
    const queryText = `
    ...
    pool.query(queryText, queryArguments)
    .then(()=>{
        if(verbose)console.log('Genre inserted');
        res.sendStatus(200);        
    })
    .catch((error)=>{
        if(verbose)console.log('Error inserting genre', error);
        res.sendStatus(500);
    })
})// END
